<!-- ========================= HERO ========================= -->
<p align="center">
  <img width="100%" src="https://capsule-render.vercel.app/api?type=waving&height=300&color=0:0F2027,50:203A43,100:2C5364&text=Aasish%20Karki&fontColor=ffffff&fontSize=55&fontAlignY=35&animation=fadeIn&desc=Full%20Stack%20Developer%20|%20Blender%203D%20Artist&descAlignY=55"/>
</p>

<p align="center">
  <img src="https://readme-typing-svg.herokuapp.com?font=JetBrains+Mono&weight=700&size=24&duration=2500&pause=1000&color=58A6FF&center=true&vCenter=true&width=850&lines=Welcome+to+my+GitHub!;Full+Stack+Django+Developer;React+%26+Next.js+Developer;Python+Backend+Engineer;Blender+3D+Artist;Always+Learning+New+Technologies"/>
</p>

<!-- Navigation -->
<p align="center">
  <a href="#about-me">About</a> • 
  <a href="#tech-stack">Tech Stack</a> • 
  <a href="#projects">Projects</a> • 
  <a href="#github-dashboard">Dashboard</a> • 
  <a href="#contact">Contact</a>
</p>

---

<a name="about-me"></a>
# 👨‍💻 About Me
<table>
  <tr>
    <td width="55%">
      <h3>Hello!</h3>
      <p>I'm <b>Aasish Karki</b>, a passionate Full Stack Developer from Nepal. I enjoy building modern, scalable web applications using <b>Python</b>, <b>Django</b>, <b>React</b>, and <b>Next.js</b>, while also blending my creativity into <b>3D modeling and animation</b> using Blender.</p>
      
      <h3>🌱 Currently</h3>
      <ul>
        <li>🔥 Learning Advanced Django & System Design</li>
        <li>🚀 Building Full Stack Projects</li>
        <li>🎨 Creating Blender Assets</li>
        <li>⚽ Football Goalkeeper</li>
      </ul>
    </td>
    <td width="45%">
      <img src="https://media.giphy.com/media/ZVik7pBtu9dNS/giphy.gif" width="100%">
    </td>
  </tr>
</table>

---

<a name="tech-stack"></a>
# 🚀 Tech Stack

### Languages
<p><img src="https://skillicons.dev/icons?i=python,javascript,typescript,c,cs"/></p>

### Frontend
<p><img src="https://skillicons.dev/icons?i=react,nextjs,html,css,tailwind"/></p>

### Backend
<p><img src="https://skillicons.dev/icons?i=django,nodejs,postgres,mongodb"/></p>

### Tools
<p><img src="https://skillicons.dev/icons?i=git,github,docker,vscode,blender"/></p>

---

<a name="projects"></a>
# 🌟 Featured Projects

| Project | Description |
| :--- | :--- |
| [**Green Basket**](https://github.com/GK-Aasish/green-basket) | Agriculture Marketplace built using Django |
| [**Portfolio**](https://github.com/GK-Aasish/portfolio) | Modern Full Stack Portfolio |
| [**Student Result System**](https://github.com/GK-Aasish/student-result-system) | Django Project with Proper Database Design |

---

<a name="github-dashboard"></a>
# 📊 GitHub Dashboard
<p align="center">
  <a href="https://github.com/GK-Aasish">
    <img src="https://github-readme-stats.vercel.app/api?username=GK-Aasish&show_icons=true&theme=tokyonight&hide_border=true" alt="Stats"/>
  </a>
  <a href="https://github.com/GK-Aasish">
    <img src="https://github-readme-streak-stats.herokuapp.com/?user=GK-Aasish&theme=tokyonight&hide_border=true" alt="Streak"/>
  </a>
</p>
<p align="center">
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=GK-Aasish&layout=compact&theme=tokyonight&hide_border=true"/>
</p>

---

# 🐍 Contribution Snake
<p align="center">
  <img src="https://raw.githubusercontent.com/GK-Aasish/GK-Aasish/output/github-contribution-grid-snake.svg?color_snake=gold&color_dots=#161b22,#f0f0f0,#fcfc03,#fcfc03,#fcfc03" width="600"/>
</p>

---

<a name="contact"></a>
# 📫 Connect With Me
<p align="center">
  <a href="mailto:gk.aasishkarki@gmail.com">
    <img src="https://img.shields.io/badge/Gmail-D14836?style=for-the-badge&logo=gmail"/>
  </a>
  <a href="https://github.com/GK-Aasish">
    <img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github"/>
  </a>
</p>

<p align="center">
  <img src="https://komarev.com/ghpvc/?username=GK-Aasish&style=for-the-badge&color=blueviolet"/>
</p>

---
<p align="center">
  > <b>"First, solve the problem. Then, write the code."</b>
</p>

<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&section=footer&height=180&color=0:2C5364,100:0F2027"/>
</p>
